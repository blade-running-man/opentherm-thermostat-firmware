...
| `tools/` | `opentherm_ids.py` — the single list of entities; `generate_registry.py` — the generator; `registry_model.py`, `render_c.py`, `render_ts.py` and `render_discovery.py` — the generator's model and its C / TS / discovery renderers; `regenerate.py` — the pre-build hook; `tests/` — pytest, including `test_source_guards.py` and `test_source_guards_mqtt.py` (the source guards), `test_discovery.py` (the HA-kill rules on the rendered documents) and `test_ha_schema.py` (every document handed to Home Assistant's own `DISCOVERY_SCHEMA` — needs `HA_PY`) |
